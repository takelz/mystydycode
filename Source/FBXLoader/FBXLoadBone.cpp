...
void GetBone(FbxScene *pScene, BoneListInfo &boneListInfo)
{
	...
	while (it != boneListInfo.list.end()) {
		...
		switch (order) {
		case FbxEuler::EOrder::eOrderXYZ: info.order = RotateOrder::XYZ; break;
		case FbxEuler::EOrder::eOrderZXY: info.order = RotateOrder::ZXY; break;
