...
void GetMaterial(FbxScene *pScene, MaterialList &materialList)
{
	...
	while (it != materialList.end()) {
		...
		if (pMaterial->GetClassId().Is(FbxSurfacePhong::ClassId)) {
			...
			// 透過度
			FbxPropertyT<FbxDouble3> lTransparent = pPhong->TransparentColor;
			phong->transparency = (float)lTransparent.Get()[0];
		} else {
			info.pPhong = NULL;
		}
		...
		if (1 < layeredTextureCount) {
			LogOutput("Not support [LayeredTexture]\n");
			assert(false); // 未対応
		} else {
			...
			if (fileTextureCount > 0) {
				for (int i = 0; i < fileTextureCount; ++i) {
					...
					if (texture) {
						it->texture = texture->GetFileName();
